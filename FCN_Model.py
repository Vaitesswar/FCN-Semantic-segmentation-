...
class decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(self.deconv1(pool_5)) # 7 x 7 -> 14 x 14 (Input image size: 224 x 224)
        x = self.bn1(x + pool_4) # 14 x 14
        x = self.relu(self.deconv2(x)) # 14 x 14 -> 28 x 28
        x = self.bn2(x + pool_3) # 28 x 28
        x = self.bn3(self.relu(self.deconv3(x))) # 28 x 28 -> 56 x 56
        x = self.bn4(self.relu(self.deconv4(x))) # 56 x 56 -> 112 x 112
        x = self.bn5(self.relu(self.deconv5(x))) # 112 x 112 -> 224 x 224
        x = self.score(x)
